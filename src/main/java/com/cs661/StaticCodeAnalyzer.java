...
/**
 * Week 7: Analysis Rule 1 – Detect variables declared but never used.
 * Uses scope stack (class / method / block).
 */
public class StaticCodeAnalyzer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        new ScopeVisitor().visit(cu, null);

        scopeStack.pop();

        // ---- Report: Declared but never used ----
        ...
        for (VariableInfo v : allVariables) {
            if (!v.used) {
                any = true;
                System.out.printf(
                        "⚠️  %s:%d - Variable '%s' in scope [%s] is declared but never used.%n",
    ...
    private static class ScopeVisitor extends VoidVisitorAdapter<Void> {
        ...
        public void visit(VariableDeclarator var, Void arg) {
            ...
            if (scope.variables.containsKey(name)) {
                System.out.printf(
                        "WARNING: Redeclaration of '%s' in same scope at line %d%n",
                        name, line
                );
                return;
            }
            ...
            if (outer != null) {
                ...
                if (outerInfo != null) {
                    System.out.printf(
                            "INFO: '%s' at line %d shadows variable from scope [%s]%n",
                            name, line, outerInfo.scopePath
                    );
                }
            }
